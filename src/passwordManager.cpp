...
PasswordManager_SQLite3::PasswordManager_SQLite3() {
  ...
  sqlite3_config(SQLITE_CONFIG_LOG, &PasswordManager_SQLite3::errorLogCallback,
                 NULL);
}
...
char **PasswordManager_SQLite3::_getQuery(const char *sqlRequest,
                                          int32_t *pnRow, int32_t *pnCol) {
  char **query = NULL;
  char *errMsg = NULL;
  int32_t err;
...
int32_t PasswordManager_SQLite3::openDB(const std::string &filename) {
  int32_t err =
      sqlite3_open_v2(filename.c_str(), &_conn,
                      SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE, NULL);
  if (err != SQLITE_OK) {
    (*_os) << "An error occured while open database file: " << filename
           << ".  error_code: " << err << " " << sqlite3_errstr(err)
           << std::endl;
    return err;
  }
  return _initDB();
...
int32_t PasswordManager_SQLite3::showIndexByDomain(const std::string &domain) {
  std::ostringstream sql;
  sql << "SELECT " << COLUMN[0].name << "," << COLUMN[1].name << "FROM "
      << TABLENAME << " WHERE " << COLUMN[1].name << R "( LIKE '%)" << domain
      << R "(%')";
  sqlite3_stmt *pstmt;
  int32_t rc;
...
bytes PasswordManager_SQLite3::searchByIndex(int32_t id) {
  std::ostringstream sql;
  sql << "SELECT " << COLUMN[2].name << "," << COLUMN[3].name << " FROM "
      << TABLENAME << " WHERE " << COLUMN[0].name << " = ?";
  sqlite3_stmt *pstmt;
  int32_t rc;
  int32_t nblob;
  bytes ret;
...
int32_t PasswordManager_SQLite3::addPasswd(bytes passwd,
...
void PasswordManager_SQLite3::errorLogCallback(void *pArg, int iErrCode,
                                               const char *zMsg) {
  (void)pArg;
