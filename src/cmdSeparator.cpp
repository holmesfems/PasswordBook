...
void CmdSeparator::initCmdDict() {
  registerCmd("generate", &CmdSeparator::_cmd_generate,
...
CmdSeparator::~CmdSeparator() {
  _pwdGenerator->save(generatorConfigFile.string());
}
...
#ifdef DEBUG
  std::for_each(split.begin(), split.end(),
...
inline void CmdSeparator::registerCmd(std::string cmd,
                                      std::function<int(Params &)> handler,
